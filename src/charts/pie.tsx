...
import { AmbientLight, AxesHelper, Color, DirectionalLight, DoubleSide, ExtrudeGeometry, Group, Mesh, MeshBasicMaterial, MeshLambertMaterial, type Object3D, PerspectiveCamera, PlaneGeometry, RepeatWrapping, Scene, Shape, TextureLoader, Vector3, WebGLRenderer } from "three";
...
class Pie {
    ...
    private createPie() {
        ...
        for (var i = 0; i < data.length; i++) {
            let percent = data[i].value / count
            if (i == 0) {
                startAngle = 0
            } else {
    ...
    private addRing(opt: Partial<RingOption>) {
        ...
        outerShape.arc(0, 0, option.outerRadius, option.startAngle, option.endAngle)
        const outerPoints = outerShape.getPoints(option.segments)
        // 内层：需要把开始结束角度调换下，并反向绘制
        const innerShape = new Shape()
        innerShape.arc(0, 0, option.innerRadius, option.endAngle, option.startAngle, true)
    ...
    private chooseRing(activeIndex = 0, isFirst = false) {
        const prevIndex = activeIndex - 1 < 0 ? this.#option.data.length - 1 : activeIndex - 1
    ...
    destroy() {
        if (this.#prevMesh) {
            ...
            gsap.set((this.#prevMesh as Mesh).material, { opacity: this.#option.opacity })
        }
        this.stop()
...
interface PieProps {
    ...
    delay?: number;
    loopComplete?: () => void;
    children?: (val: NonNullable<PieProps['data']>[number] & { count: number }) => JSXElement;
}
...
export default function PieChart({
    ...
    const count = data.map((item) => item.value).reduce((prev, current) => prev + current, 0)
